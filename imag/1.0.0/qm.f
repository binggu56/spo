...
C
      open(44,file='pot')
      open(54,file='wf0.dat')
      open(55,file='wft.dat')
      open(53,file='corr')
      open(56,file='rho')
      open(57,file='prob')
      open(103,file='prod')
      open(108, file = 'ave.dat')
...
c     if(y.gt.ai)  v(i)=v(i)-im*di*(y-ai)**2
      psi0(i) = an*exp(-al*(x(i)-x0)**2+im*p0*(x(i)-x0))
      psi(i) = psi0(i)
...
! ---- propagation --------------------        
      ...
      DO 2 J=1,NT
        ...
        xav = 0d0
        do i=1,n1
          xav = xav+x(i)*abs(psi(i))**2*dx
        enddo

        write(108,1000) abs(t),xav
!	      write(57,1000) t,cm1,cm2,abs(c1),abs(c2),c0,as
        write(53,1000) abs(t),c1,c2
!        write(56,1000) t,c0,as,as/cm1-(c0/cm1)**2,cm1
         if(c0.gt.csave) csave=c0
2      CONTINUE

c	write(57,1000) e0,csave,c0
! ---- renormalization, print out final wavepacket

        anm = 0d0 
        do i=1,n1
          anm = anm + abs(psi(i))**2*dx        
        enddo 
        do i=1,n1
          write(55,1000) x(i),abs(psi(i))**2/anm 
        enddo
...
11      cwf(i)=cwf(i)*exp(-ts*ak2(i))
        ...
        nim=(0.d0,-1.d0)
         do 11 i=1,ni
        if(ts .ge. 0) cwf(i)=cwf(i)*exp(-ts*v(i))
        if(ts .lt. 0) cwf(i)=cwf(i)*exp(-ts*conjg(v(i)))
11      continue
